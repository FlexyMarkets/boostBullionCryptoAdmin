...
function LoginPasswordUpdate() {
    ...
    const { register, handleSubmit, reset, formState: { errors } } = useForm({
        resolver: zodResolver(loginPasswordUpdateSchema),
        defaultValues
    });
    ...
    const [userLoginPasswordUpdate, { isLoading }] = useUserLoginPasswordUpdateMutation()

    const onSubmit = async (data) => {

        try {

            const response = await userLoginPasswordUpdate(data).unwrap();
    ...
    return (
        <Stack mt={"2rem"}>
            <Typography variant='h5' fontWeight={"bold"} mb={"2rem"}>Change logIn password</Typography>
            <Card
                component={"form"}
                sx={{
